...
def scan_and_mark_completed_members(caller, config, list_file=None, progress_file=None, compare_remote=True, dry_run=True, download_missing=False, download_limit=None, candidate_set=None):
    ...
    - compare_remote: if True, fetch remote member image id list to determine exact remote set
    ...
    - 不用远程(compare_remote=False)：
      - 只对“本地/DB 已经有作品的作者（present>0）”进行完成判定；
      - 完成判定仅基于 DB 的 total_images（来自 5a 预热/历史远端计数），若缺失则不自动标完成。
      - 不会下载任何内容。
    - 用远程(compare_remote=True)：
      - 仍然先以“本地/DB 已经有作品的作者（present>0）”为集合再远程比对；
      - 远端返回 totalImages 时写回 DB.total_images 作为下次“不用远程”的基准；
      - 仅当拿到 remote_ids 时才会计算 missing_ids 并按需下载（避免误下载）。
    """
    br: PixivBrowser = caller.__br__
    db = caller.__dbManager__
    ...
    try:
        ...
        # read new small-config options (backwards compatible)
        candidate_set = getattr(config, 'menu5CandidateSet', None) or None
        require_present = getattr(config, 'menu5RequirePresent', True)
        config_download_missing = getattr(config, 'menu5DownloadMissing', False)
        # allow function param to enable download_missing but honor config as default
        download_missing = download_missing or config_download_missing

        # base followed set (F) from local list if present
        F_set = set()
        try:
            F_set = set(int(x.memberId) for x in total_list if getattr(x, 'memberId', None) is not None)
        except Exception:
            F_set = set()

        # downloaded/local set (L)
        try:
            L_members = db.selectDistinctMembersWithImages() if hasattr(db, 'selectDistinctMembersWithImages') else []
            L_set = set(int(x) for x in L_members)
        except Exception:
            L_set = set()

        # remote set (R) will be fetched later only if needed (after reading progress)
        R_set = set()
        ...
        # now compute candidate ids by requested simple set names (need done_set)
        # ensure remote fetch if candidate requires it
        cs = (candidate_set or getattr(config, 'menu5CandidateSet', 'downloaded'))
        if cs in ('remote', 'downloaded_and_remote', 'remote_not_downloaded'):
            if not compare_remote:
                PixivHelper.print_and_log('info', f"Scan: candidate '{cs}' requires remote fetch, enabling compare_remote.")
                compare_remote = True
        # fetch R only if needed
        if compare_remote and cs in ('remote', 'downloaded_and_remote', 'remote_not_downloaded'):
            try:
                r_list = []
                r_list.extend(get_bookmarks(caller, config, False, 1, 0, br._myId)[0])
                r_list.extend(get_bookmarks(caller, config, True, 1, 0, br._myId)[0])
                R_set = set(int(x.memberId) for x in r_list if getattr(x, 'memberId', None) is not None)
            except Exception as ex:
                PixivHelper.print_and_log('warn', f"Scan: failed to fetch remote followed list: {ex}")
                R_set = set()

        candidate_ids = set()
        if cs == 'downloaded':
            candidate_ids = set(L_set)
        elif cs == 'followed':
            candidate_ids = set(F_set)
        elif cs == 'remote':
            candidate_ids = set(R_set)
        elif cs == 'downloaded_and_remote':
            candidate_ids = L_set.intersection(R_set)
        elif cs == 'followed_not_done':
            candidate_ids = set(F_set) - set(int(x) for x in done_set) if done_set else set(F_set)
        elif cs == 'downloaded_not_done':
            candidate_ids = set(L_set) - set(int(x) for x in done_set) if done_set else set(L_set)
        elif cs == 'remote_not_downloaded':
            candidate_ids = set(R_set) - set(L_set)
        elif cs == 'done':
            candidate_ids = set(int(x) for x in done_set)
        else:
            candidate_ids = set(L_set)

        # enforce require_present: only keep those with local present files
        if require_present:
            candidate_ids = candidate_ids.intersection(L_set)

        PixivHelper.print_and_log('info', f"Scan: Candidate set={cs}, require_present={require_present}, candidates={len(candidate_ids)}")

        # rebuild total_list as simple objects with memberId so rest of code works unchanged
        from types import SimpleNamespace
        total_list = [SimpleNamespace(memberId=m) for m in sorted(candidate_ids)]

        if len(total_list) == 0:
            PixivHelper.print_and_log('info', 'Scan: No candidate members after applying candidate_set and require_present.')
            return

        member_ids = [int(x.memberId) for x in total_list if getattr(x, 'memberId', None) is not None]

        # load total_images from DB (as baseline remote count)
        total_map = {}
        try:
            if hasattr(db, 'selectMemberTotalImagesMap'):
                total_map = db.selectMemberTotalImagesMap(member_ids)
        ...
        # when compare_remote=False, never download
        if not compare_remote and download_missing:
            PixivHelper.print_and_log('info', 'Scan: compare_remote is disabled, forcing download_missing=False (no remote IDs, cannot safely download).')
        ...
        for item in total_list:
            ...
            present_cnt = len(present_ids)

            # 关键：只处理本地已有的作者
            if present_cnt <= 0:
                # 不做远端请求、不做标记（避免“远程对全部关注作者对比”）
                continue
            ...
            if compare_remote:
                ...
                # 写回 DB 的远端数量（用于下次 compare_remote=False 的本地对比）
                if remote_total is not None:
                    try:
                        db.updateMemberTotalImages(int(mid), int(remote_total))
                        # keep total_map in sync within this run
                        if total_map is not None:
                            total_map[int(mid)] = int(remote_total)
                    except Exception as ex:
                        PixivHelper.print_and_log('warn', f'Scan: failed to update DB total_images for {mid}: {ex}')
            ...
            if missing_ids:
                ...
                PixivHelper.print_and_log('info', f'Scan: Member {mid} no missing images detected (local={present_cnt}, remote={len(remote_ids) if remote_ids else remote_total}).')

            # optionally download missing images (ONLY when we have exact remote_ids)
            downloaded_count = 0
            ...
            if compare_remote:
                # 远端模式：优先用 remote_ids 精确判定；没有 remote_ids 时退化到 remote_total（仅计数）
                if remote_ids:
                    is_complete = remote_ids.issubset(present_ids)
                elif remote_total is not None:
                    try:
                        is_complete = len(present_ids) >= int(remote_total)
                    except Exception:
                        is_complete = False
                else:
                    is_complete = False
            else:
                # 本地模式：只用 DB total_images（历史远端计数）判定；缺失则不标完成
                baseline = total_map.get(int(mid)) if total_map is not None else None
                if baseline is not None:
                    try:
                        baseline_int = int(baseline)
                        if baseline_int > 0 and len(present_ids) >= baseline_int:
                            is_complete = True
                    except Exception:
                        is_complete = False
...
def _is_member_complete_via_db(caller, config, member_id, skip_local_scan=False, compare_remote=False):
    ...
    返回: (is_complete, present_ids_count, remote_ids_count_or_total)

    约束（按 5b 的需求）：

    - compare_remote=False 时：只允许在 DB.total_images 存在且 present>=total_images 时标记完成；
      DB 未记录 total_images（0/None）时不标记完成（避免误判）。
    - compare_remote=True 时：优先用 remote_ids 精确判定；拿不到 remote_ids 时退化到 remote_total 计数判定。
    """
    db = caller.__dbManager__
    br = caller.__br__
    ...
    if compare_remote:
        ...
        # 本地模式：必须依赖 DB.total_images
        try:
            row = db.selectMemberByMemberId(int(member_id))
            if row is not None and len(row) > 5:
                total_images_in_db = row[5]  # column for total_images
                if total_images_in_db is not None:
                    total_images_in_db = int(total_images_in_db)
                if total_images_in_db and total_images_in_db > 0:
                    if len(present_ids) >= total_images_in_db:
                        is_complete = True
        except Exception:
            pass
